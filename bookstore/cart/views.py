...
def cart_show(request):
	...
	for id, count in res_dict.items():
		...
		totle_count += int(count)
		totle_price += int(count)*books.price
...
def cart_update(request):
	if not request.seesion.get('islogin'):
		return JsonResponse({'res':0, 'errmsg':'请先登录'})
	...
	try:
		...
		return JsonResponse({'res':3, 'errmsg':'商品数必须为整数'})
